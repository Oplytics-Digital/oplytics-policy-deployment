...
export const breakthroughObjectives = mysqlTable("breakthrough_objectives", {
  ...
  category: mysqlEnum("boCategory", ["safety", "cost", "delivery", "quality", "morale"]).notNull(),
...
export const improvementProjects = mysqlTable("improvement_projects", {
  ...
  category: mysqlEnum("projCategory", ["strategic", "operational", "improvement"]).default("improvement").notNull(),
  sortOrder: int("sortOrder").default(0).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
