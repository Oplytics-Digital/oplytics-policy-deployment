...
async function seed() {
  ...
  if (existingPlans.length > 0) {
    ...
    const boData = [
      ...
      { code: "M1", description: "Build a continuous improvement culture with 80%+ engagement across all BUs", category: "morale", sortOrder: 5 },
    ];
    ...
    const projData = [
      { code: "P1.1", name: "Middleton foam scrap reduction", description: "Root-cause analysis and process parameter optimisation on MID-FM1 and MID-FM2 foam-making machines to reduce off-spec blocks", ownerName: "Robin Parry", status: "on-track", progress: 62, startDate: "2026-01-15", endDate: "2026-06-30", category: "improvement", sortOrder: 1 },
      { code: "P1.2", name: "Dukinfield conversion line waste mapping", description: "Value-stream mapping of DUK-CV1 and DUK-CV2 conversion lines to identify and eliminate waste in the lamination and bonding process", ownerName: "James Henderson", status: "on-track", progress: 45, startDate: "2026-02-01", endDate: "2026-08-31", category: "improvement", sortOrder: 2 },
      { code: "P1.3", name: "F&B division safety audit programme", description: "Design and deploy standardised safety audit checklists, near-miss reporting, and monthly review cadence across all 9 Furniture & Bedding sites", ownerName: "Rachel Attwood", status: "at-risk", progress: 35, startDate: "2026-01-01", endDate: "2026-12-31", category: "operational", sortOrder: 3 },
      { code: "P1.4", name: "Poznan mattress line ramp-up", description: "Commission MPL-ML1 mattress assembly line in Poznan, train local operators, and achieve 80% utilisation within 6 months", ownerName: "Omar Hoek", status: "at-risk", progress: 28, startDate: "2026-03-01", endDate: "2026-09-30", category: "strategic", sortOrder: 4 },
      { code: "P1.5", name: "OEE digital rollout — Almelo & Essen", description: "Install OEE sensors and dashboards on ALM-FL1, ALM-CT1, and ESS-FL1 machines; integrate with Oplytics OEE Manager platform", ownerName: "Markus Westerkamp", status: "on-track", progress: 55, startDate: "2026-02-01", endDate: "2026-07-31", category: "strategic", sortOrder: 5 },
      { code: "P1.6", name: "SMED programme — Bedford & Corby cutting lines", description: "Apply Single-Minute Exchange of Dies methodology to BED-FM1 and COR cutting lines to reduce changeover from 42 min to target 27 min", ownerName: "Laura Whitfield", status: "off-track", progress: 18, startDate: "2026-01-15", endDate: "2026-07-31", category: "improvement", sortOrder: 6 },
      { code: "P1.7", name: "Vita CI Academy launch", description: "Develop and deliver continuous improvement training modules (5S, problem solving, standard work) to 200+ operators across all 4 business units", ownerName: "Cyril Wasem", status: "on-track", progress: 40, startDate: "2026-02-01", endDate: "2026-11-30", category: "operational", sortOrder: 7 },
    ];
    ...
    const correlations = [
      ...
      // AO ↔ Project (ao-proj)
      { sourceId: aoIds["T1"], targetId: projIds["P1.1"], sourceType: "ao", targetType: "project", strength: "strong", quadrant: "ao-proj" },
      { sourceId: aoIds["T1"], targetId: projIds["P1.2"], sourceType: "ao", targetType: "project", strength: "strong", quadrant: "ao-proj" },
      ...
      { sourceId: projIds["P1.1"], targetId: kpiIds["IP1.1"], sourceType: "project", targetType: "kpi", strength: "strong", quadrant: "proj-kpi" },
      { sourceId: projIds["P1.2"], targetId: kpiIds["IP1.1"], sourceType: "project", targetType: "kpi", strength: "weak", quadrant: "proj-kpi" },
      { sourceId: projIds["P1.3"], targetId: kpiIds["IP1.4"], sourceType: "project", targetType: "kpi", strength: "strong", quadrant: "proj-kpi" },
      { sourceId: projIds["P1.4"], targetId: kpiIds["IP1.6"], sourceType: "project", targetType: "kpi", strength: "strong", quadrant: "proj-kpi" },
      { sourceId: projIds["P1.4"], targetId: kpiIds["IP1.3"], sourceType: "project", targetType: "kpi", strength: "strong", quadrant: "proj-kpi" },
      { sourceId: projIds["P1.5"], targetId: kpiIds["IP1.2"], sourceType: "project", targetType: "kpi", strength: "strong", quadrant: "proj-kpi" },
      { sourceId: projIds["P1.6"], targetId: kpiIds["IP1.5"], sourceType: "project", targetType: "kpi", strength: "strong", quadrant: "proj-kpi" },
      { sourceId: projIds["P1.7"], targetId: kpiIds["IP1.7"], sourceType: "project", targetType: "kpi", strength: "strong", quadrant: "proj-kpi" },
      // KPI ↔ BO (kpi-bo)
